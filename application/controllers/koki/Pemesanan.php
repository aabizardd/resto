...
class Pemesanan extends MY_Controller {
    ...
    public function detail_data($id){
        ...
        $data['admincontent'] = "$this->url1/$this->url2/detail-data";
        $this->load->view('admin_template', $data); 
    }
    ...
    {
        // try {
        //     $this->load->library('ReceiptPrintKoki');
        //     $this->receiptprintkoki->connect(''.$this->tb_printer->dataprinter()->row()->ip_printer.'', $this->tb_printer->dataprinter()->row()->port_printer);
        //     $this->receiptprintkoki->print_test_receipt($id);
        //     echo "<script>setTimeout('window.close()');</script>"; 
             
        // } catch (Exception $e) {
        //     print_r($e);
        //   log_message("error", "Error: Could not print. Message ".$e->getMessage());
        //   $this->receiptprintkoki->close_after_exception();
        // }                     
    }
...
/* Location: ./application/controllers/koki/Pemesanan.php */