...
class ReceiptPrintBackup {
  ...
  public function print_test_receipt($text = "")
  {
    ...
    $this->printer->selectPrintMode();
    $this->printer -> text($this->columnify($leftCol, $rightCol, 22, 22, 4));
    $this->printer->selectPrintMode(Printer::MODE_DOUBLE_WIDTH);
    $this->add_line("========================"); // blank line
    $this->printer->selectPrintMode();
    ...
    $this->printer -> setJustification(Printer::JUSTIFY_LEFT);
    $this->printer -> setEmphasis(true);
    $this->printer->text(new item('Menu','Jumlah','Harga','Subtotal')); // blank line
    $this->printer -> setEmphasis(false);
    foreach ($items as $item) {
        $this->printer -> setJustification(Printer::JUSTIFY_LEFT);
        $this->printer -> text(new item($item['nama_produk'],$item['jumlah_transaksi'],number_format($item['harga_transaksi'], 0, ',', '.'),number_format($item['total_transaksi'], 0, ',', '.')));
    }
    $this->printer->selectPrintMode(Printer::MODE_DOUBLE_WIDTH);
    $this->add_line("========================"); // blank line
    $this->printer->selectPrintMode();
